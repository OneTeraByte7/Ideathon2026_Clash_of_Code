...
const CustomTooltip = ({ active, payload }) => {
  ...
  const d = payload[0];
  return (
    <div className="rounded-lg px-3 py-2 font-mono text-xs" style={{ background: "#0d1220", border: "1px solid rgba(0,245,212,0.2)", color: "#00f5d4" }}>
      <div className="text-white opacity-50 mb-1">{new Date(d.payload.timestamp).toLocaleTimeString()}</div>
      <div>Risk: <span className="font-bold" style={{ color: d.value >= 70 ? "#ff2d78" : d.value >= 40 ? "#f59e0b" : "#00f5d4" }}>{d.value}</span></div>
    </div>
  );
};
      ...
      {patient && (
        <>
          ...
          <motion.div
            ...
            style={{ background: "#080c14", borderLeft: `1px solid ${accent}22` }}
          >
            {/* Top accent bar */}
            ...
            <div className="p-7">
              ...
              <div className="flex items-center gap-5 mb-6">
                ...
                <div>
                  ...
                  <h2 className="font-display font-bold text-2xl text-white">{patient.name}</h2>
                  <p className="font-mono text-xs opacity-40 mt-1">{patient.diagnosis}</p>
              ...
              {/* Vitals grid */}
              <div className="mb-6">
                <div className="font-mono text-xs tracking-widest opacity-30 mb-3 uppercase">Current Vitals</div>
              ...
              {/* Trend chart */}
              <div className="mb-6">
                <div className="font-mono text-xs tracking-widest opacity-30 mb-3 uppercase">Risk Score Trend</div>
                <div className="rounded-xl p-4" style={{ background: "#0d1220", border: "1px solid rgba(255,255,255,0.04)" }}>
                  {chartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={160}>
                      <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                        ...
                        <YAxis domain={[0, 100]} tick={{ fill: "rgba(255,255,255,0.2)", fontSize: 10, fontFamily: "JetBrains Mono" }} axisLine={false} tickLine={false} />
                        <Tooltip content={<CustomTooltip />} />
                        <Area type="monotone" dataKey="score" stroke={accent} strokeWidth={2} fill="url(#scoreGrad)" dot={false} />
                      </AreaChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="h-40 flex items-center justify-center font-mono text-xs opacity-20">NO DATA YET</div>
                  )}
                </div>
              </div>
              ...
              {patient.active_alerts?.length > 0 && (
                <div>
                  <div className="font-mono text-xs tracking-widest opacity-30 mb-3 uppercase">Active Alerts</div>
